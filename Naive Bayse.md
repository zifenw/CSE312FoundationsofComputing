## Naive Bayes
使用朴素贝叶斯分类器实现垃圾邮件过滤器，该过滤器从我们预先标记的训练数据中学习单词垃圾邮件概率，然后预测一组以前从未见过的电子邮件的标签（火腿或垃圾邮件）。
    ...
    def fit(self, train_hams:List[str], train_spams:List[str]):
        ...
        def get_counts(filenames:List[str]) -> Dict[str, int]:
            ...
            for filename in filenames:
                words = self.word_set(filename)  # Get unique words in the email
                for word in words:
                    counts[word] = counts.get(word, 0) # Count occurrences of each word
            return counts
        ...
        # Calculate word frequencies for ham and spam
